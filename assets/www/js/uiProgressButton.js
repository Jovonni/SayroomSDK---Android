...
;( function( window ) {
	...
	UIProgressButton.prototype._initEvents = function() {
		...
		var callback = false;
		self._submit(e, callback); 
		} );
	}

	UIProgressButton.prototype._submit = function(e, callback) {
			...
			endLoading = function() {
				...
				if( typeof status === 'number' ) {
					var statusClass = 'success',
						statusEl = status = self.successEl;
						
						if(window.SC(homeScope).recordPane)
						window.SC(homeScope).UI.recordPaneTearDown();
					...
					setTimeout( function() {
						...
						if(window.SC(homeScope).recordPane){
						   window.SC(homeScope).setVocalizationsPane(window.SC(homeScope).selectedId);
						}else if(window.SC(homeScope).taskPane){
						   window.SC(homeScope).updateTaskCompletion();
						}
