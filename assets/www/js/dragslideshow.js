...
;( function( window ) {
	...
	DragSlideshow.prototype._initEvents = function() {
		...
		this.slides.forEach( function( slide ) {
			...
			// reveal content
			slide.querySelector( 'button.content-switch' ).addEventListener( 'touchstart', function() { self._toggleContent( slide ); } );
		} );
	...
	DragSlideshow.prototype._getContentPage = function( slide ) {
		console.log('div.content[data-content = "content-1"]');
	...
	DragSlideshow.prototype._toggleContent = function( slide ) {
		...
		if( this.isContent ) {
			window.SC(tutorialScope).enterSayRoom();
			//return;
...
var tutorial = function(){
      return (function() {
		  tutorialScope = document.getElementsByClassName('tut')[0];
...
tutorial.prototype.endTutorial = function(){
  ...
  classie.add(document.getElementById( 'slideshow' ),'hide');
